...
activitylist = [
    ("1.", "Chess Club", "No Cost"),
    ("2.", "Swimming Lessons", "\u00A33"),
    ("3.", "Basketball Team", "\u00A32"),
    ("4.", "Science Club", "No Cost"),
    ("5.", "Drama Club", "\u00A32"),
    ("6.", "Art Classes", "\u00A32"),
    ("7.", "Music Lessons - Guitar", "\u00A34"),
    ("8.", "Dance Workshop", "\u00A32"),
    ("9.", "Football Team", "\u00A32"),
    ("10.", "Robotics Club", "\u00A33"),
    ("11.", "Yoga Sessions", "\u00A31"),
    ("12.", "Cooking Classes", "\u00A37"),
    ("13.", "Gardening Club", "No Cost"),
    ("14.", "Mathematics Olympiad Preparation", "No Cost"),
    ("15.", "Creative Writing Workshop", "\u00A35"),
    ("16.", "Debate Team", "No Cost"),
    ("17.", "Karate Classes", "\u00A33"),
    ("18.", "Photography Club", "\u00A32"),
    ("19.", "Environmental Club", "No Cost"),
    ("20.", "Chess Tournament Preparation", "\u00A31")]

                    
def makeBooking():
    print()
    for Num,Name,Price in activitylist: 
    ...
    while validactivity == False:
        try:
            activitychoice = input("Which activity would you like to book? Please choose a number from the list above: ")
            if activitychoice == "quit" or activitychoice == "Quit" or activitychoice == "QUIT":
                return
            elif int(activitychoice) < 1 or int(activitychoice) > 20:
                 print("This number is not on the list.")
            else:
                activity = activitylist[int(activitychoice) - 1]
                validactivity = True
        except:
    ...
    booking = (name , str(activity[1]) , str(weekday[1]) , str(date[1]) , str(time[1]) , email)
...
def processPayment(activity):
    _, _, cost = activity
    if cost != "No Cost":
        print(f"The cost for this activity is {cost}.")
        while True:
        ...
        print("This activity is free. No payment required. Your booking is confirmed.")
...
while 1==1:
    print()
    print("Welcome to the Extra-Curricular activities Portal! Here you can select an activity, then select a date and time. Say 'quit' at any time to exit the process.")
    print()
